...
def createPassword(passlen:int)->str:
    ...
    while 1:
        password="".join([random_letter() for _ in range(passlen)])
...
def validate_phone(phonenumber:int)->bool:
    ...
    if not phonenumber.isdigit():
        return False
    return True
def RecorreLista(lista:list)->list:
    return [setUser(i) for i in lista]
def getPhoneNumber(users:list):
    for i in users:
...
if __name__=="__main__":
    ...
    users=RecorreLista(names)
    getPhoneNumber(users)
